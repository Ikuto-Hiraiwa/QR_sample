...
function um_profile_content_main( $args ) {
	...
	*/
	...
	$login_user_role = get_user_meta($login_user_id , 'user_role' , $single);
	...
	if($user_level=='um_artist_member'){
		...
		*/
		...
		$artist_last_month_point = get_user_meta($profiel_user_id , 'artist_last_month_point' , $single);
		$artist_this_month_point = get_user_meta($profiel_user_id , 'artist_this_month_point' , $single);
		$artist_last_month_rank = get_user_meta($profiel_user_id , 'artist_last_month_rank' , $single);
		$artist_rank_query = "SELECT `rank` FROM `cats_usermeta` WHERE `meta_key` = 'artist_point' AND `user_id`='" .$profiel_user_id ."';";
		...
		echo '<div align="center"><h3>総ポイント<br>', $artist_point,'P</h3></div>';
        ...
        /*if($artist_rank==9999999){
            echo '<h3>現在全国順位 : 順位無し</h3>';
        }else{
		    ...
		    if($artist_point==0){
		    	echo '<h3>現在総合全国順位 : 順位無し</h3>';
		    }else{
			echo '<h3>現在総合全国順位 : '.$array_rank[$profiel_user_id].'位</h3>';
		}
		...
		/*----------ユーザがログイン時にアーティストのページで観れる情報-----------*/

		if($login_user_id!=NULL){
			$artist_rireki = $wpdb->get_var("SELECT `meta_value` FROM `wp585502usermeta` WHERE `user_id`='".$profiel_user_id."' AND `meta_key`='vote_rireki'");
				...
				if ( array_key_exists($login_user_id, $artist_rireki_unserialize)==false ) {
   					 echo "<h3>このメンバーにはまだ投票していません。</h3><br>";
   				}else{
					foreach ($artist_rireki_unserialize as $key => $value) {	
						...
						if($key==$login_user_id){
							echo '<h3>あなたの推しランク : '.$count.'位 [ '.$value.' P]</h3><br>';
							if($count!=1){
								$up_point = $before_rank_point - $value;
								if($up_point==0){
									echo '<h4>推しランクアップまで1P</h4>';
								}else{
									echo '<h4>推しランクアップまで'.$up_point.'P</h4>';
								}
		...
		*/
		...
		/*------------------自分のページを見るときに、アーティストログイン時のみ表示する項目------------*/
		if($login_user_role=='um_artist_member' && $login_user_id == $profiel_user_id){
			echo '<br><br><h2>【投票者ランキング】</h2>';
			$artist_rireki = $wpdb->get_var("SELECT `meta_value` FROM `wp585502usermeta` WHERE `user_id`='".$login_user_id."' AND `meta_key`='vote_rireki'");
			...
			if($artist_rireki==NULL){
				...
				foreach ($artist_rireki_unserialize as $key => $value) {	
					...
					echo '<tr>
							<td>'.$count.'位</td>
							<td><a href="'.$site_url.'/web2/user/'.$get_fan_id.'">' .$get_fan_name.'</a>さん</td>
							<td>'.$value.'P</td>
						 </tr>';
	...
	//ファンユーザが投票ページから新規登録した後にリダイレクトで飛んできた際の処理 -> 投票ページにシークレットと共に飛ばす
	if(isset($_SESSION['secret'])){
		...
		//echo $qr_available;
		if($qr_available==1){
			wp_safe_redirect( $site_url.'/vote?secret='.$secret_id );
			exit();
		}
	}
	/*---------------------ここまで---------------*/
		...
		$user_last_month_point = get_user_meta($profiel_user_id , 'fan_last_month_point' , $single);
		$user_this_month_point = get_user_meta($profiel_user_id , 'fan_this_month_point' , $single);
		$user_last_month_rank = get_user_meta($profiel_user_id , 'fan_last_month_rank' , $single);

		echo '<div align="center"><h3>総投票ポイント<br>', $user_point,'P</h3></div>';
		echo '<table border="1" width="500" cellspacing="0" cellpadding="5" bordercolor="#333333">
				<tr>
	...
	/*----------ファンユーザがログイン時に自分のページのみで見れる情報----------*/
	if($login_user_role=='um_fan_member' && $login_user_id == $profiel_user_id){
			echo '<br><br><h2>【投票アーティスト履歴】</h2>';
			$fan_rireki = $wpdb->get_var("SELECT `meta_value` FROM `wp585502usermeta` WHERE `user_id`='".$login_user_id."' AND `meta_key`='rireki'");
			if($fan_rireki==NULL){
				echo '<h3>投票履歴なし</h3><br>';
			}else{




				$fan_rireki_unserialize = unserialize($fan_rireki);


				$count = 0;
				//degub
				//var_dump($fan_rireki_unserialize);
				echo '<br>';
			    ...
			    $array_oshi_rank = array();
			    foreach ($fan_rireki_unserialize as $key => $value) {	
			    	$artist_rireki = $wpdb->get_var("SELECT `meta_value` FROM `wp585502usermeta` WHERE `user_id`='".$value."' AND `meta_key`='vote_rireki'");
									$artist_rireki_unserialize = unserialize($artist_rireki);
									...
									//echo $login_user_id;
										...
										foreach ($artist_rireki_unserialize as $key => $value) {	
											...
											$count++;
											if($key==$login_user_id){
													$array_oshi_rank += array($count=>$rireki_id);
												/*
												echo 'あなたの推しランク
														'.$count.'位 [ '.$value.' P]<br>';
												if($count!=1){
			   ...
			   // var_dump($array_oshi_rank);
			    arsort($array_oshi_rank);
			    //var_dump($array_oshi_rank);
			    $array_oshi_rank = array_values($array_oshi_rank);
				...
				foreach ($array_oshi_rank as $key => $value) {	
					...
					$avatar = get_avatar($value,150);
					
					echo '<div class="container" style="border:#dcdcdc solid 1.5px; padding : 10px 10px 10px 10px;">
							<div class="row">
								<div class="col-sm-5 col-xs-5 col-md-5">
								<br><table>
									<tr><td>';
									if($key<2){
										echo '<span class="author-thumbanil">'.$avatar.'</span><br>';
										//echo $count;
									}
									echo '<h4><a href="'.$site_url.'/web2/user/'.$get_artist_id.'">' .$get_artist_name.'</a></h4></td></tr>
									<tr><td>現在'.$array_rank[$value].'位 : [累計 '.$get_artist_point.'P]</td></tr>';
									if($array_rank[$value]==1){
										echo '<td>王者</td>';
									}else{
										$next_rank = $array_rank[$value]-1;
										$next_rank_id = array_search($next_rank,$array_rank);
										$get_artist_next_point = get_user_meta($next_rank_id , 'artist_point' , $single) - $get_artist_point;
										echo '<tr><td>ランクアップまで'.$get_artist_next_point.'P</td></tr>';
									}
			...
			$avatar = get_avatar($login_user_id,150);
	}
