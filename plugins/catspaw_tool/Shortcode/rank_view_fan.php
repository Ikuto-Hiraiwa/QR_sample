...
$select_query = "SELECT * FROM `wp585502usermeta` WHERE `meta_key` = 'fan_point' ORDER BY `wp585502usermeta`.`point` DESC";
...
foreach ($result as $row) {
    ...
    $this_point = get_user_meta($profile_id , 'fan_point' , $single);
    ...
    <div class="row">
      <div class="col-sm-12 col-xs-8">';
        ...
        /*echo '<table border="1">
              <tr>        
                  <td rowspan="2"><span class="author-thumbanil"><a href="'.$url.'/user/'. $profile_login . '">'.$avatar.'</a></span></td>
                  <td>ユーザー名<br><span><a href="'.$url.'/user/'. $profile_login . '">'.get_user_meta($profile_id , 'nickname' , $single).'</a></span></td>
              </tr>       
              <tr>    
                  <td>ポイント<br><span>'.$this_point.'P</span></td>
              </tr>
          </table>';*/
...
$select_query = "SELECT * FROM `wp585502usermeta` WHERE `meta_key` = 'fan_this_month_point' ORDER BY `wp585502usermeta`.`point` DESC";
$result = $wpdb->get_results($select_query);
//デバッグ用
//var_dump($result);
...
foreach ($result as $row) {
    ...
    if($status=='approved'){
      ...
      $this_point = get_user_meta($profile_id , 'fan_this_month_point' , $single);
    if($before_point != $this_point || $before_point==NULL){
      $rank++;
    }
